...
export function TagForm({ mutation, tagId = '', visible = true, ownership, setTagFormState, orgId }) {
  ...
  return (
    <Box>
      <Text mb="2">
        <Trans>
          <b>Note:</b> Tags must not include data that would reclassify the asset above{' '}
          <b>Unclassified, Low Integrity, Low Availability (ULL)</b>
        </Trans>
      </Text>
      <Formik
        ...
        {({ handleSubmit, handleReset, handleChange }) => (
          <form onSubmit={handleSubmit}>
            <Grid gridTemplateColumns="repeat(4, 1fr)" gridRowGap="0.5rem" gridColumnGap="1.5rem" mx="1rem" mb="1.5rem">
              ...
              <OwnershipSelect ownership={ownership} handleChange={handleChange} orgId={orgId} />
              <Button
                variant="danger"
                type="reset"
                onClick={handleReset}
...
const OwnershipSelect = withSuperAdmin(({ ownership, handleChange, orgId }) => {
  return (
    <Box gridColumn={{ base: 'span 4', md: 'span 2' }}>
      <Flex align="center">
