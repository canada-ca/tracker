		...
		orgList = (
			...
			>
				{(
					...
					<ErrorBoundary
						key={`${slug}:${index}`}
						ErrorFallbackComponent={ErrorFallbackMessage}
					>
						<Flex align="center">
				...
				<Alert status="error" variant="left-accent" mb="6" borderRadius="md">
					...
					<Box flex="1">
						...
						<AlertDescription fontSize="md">
							<Trans>
								This may be due to insufficient user privileges or because the
								domain does not exist in the system.
								<br />
								You can request access to an Organization below to view the
								Domain results.
							</Trans>
						</AlertDescription>
					</Box>
		...
		guidanceResults = (
			<Box fontSize="lg">
				<Flex>
					<Text mr="1">
						<Trans>
							A DNS request for this service has resulted in the following error
							code:
						</Trans>
					</Text>
				...
				<Text>
					<Trans>
						If you believe this could be the result of an issue with the scan,
						rescan the service using the refresh button. If you believe this is
						because the service no longer exists (NXDOMAIN), this domain should
						be removed from all affiliated organizations.
					</Trans>
				</Text>
			</Box>
		)
	} else {
		const { results: webResults, timestamp: webTimestamp } =
			webScan?.edges[0]?.node || {}
		const { node: dnsResults } = dnsScan?.edges[0] || {}

		const hasNoIpAddresses = webResults && webResults.length === 0

		const hasNoDnsScans =
			!dnsScan || !dnsScan.edges || dnsScan.edges.length === 0

		const noScanData = (
			<Flex fontSize="xl" fontWeight="bold" textAlign="center" px="2" py="1">
				<Text>
					<Trans>
						No scan data is currently available for this service. You may
						request a scan using the refresh button, or wait up to 24 hours for
						data to refresh.
					</Trans>
				</Text>
			</Flex>
		...
		const noIpAddressData = (
			<>
				...
				<Flex fontSize="xl" fontWeight="bold" textAlign="center" px="2" py="1">
					<Text>
						<Trans>
							No IP addresses were available to scan for this service. Please
							check your DNS configuration or contact us for assistance.
						</Trans>
					</Text>
				</Flex>
			</>
		...
		<Flex flexDirection="column" width="100%">
			...
			>
				...
				{wildcardSibling && (
					<Badge
						ml="2"
						colorScheme={wildcardEntry ? "red" : "blue"}
						variant="subtle"
						alignSelf="center"
					>
						{wildcardEntry ? (
							<Trans>Wildcard Entry</Trans>
						) : (
							<Trans>Wildcard Sibling</Trans>
						)}
					</Badge>
				)}
