...
class RoleEnums(graphene.Enum):
    ...
    def description(self):
        if self == RoleEnums.USER_READ:
            ...
            return 'A user who has been given access to run scans, and ' \
                'manage domains'
        elif self == RoleEnums.ADMIN:
            return 'A user who has the same access as a user write account, ' \
                   'but can define new user write accounts'
        elif self == RoleEnums.SUPER_ADMIN:
            return 'A user who has the same access as an admin, but can ' \
                   'define new admins'
        else:
            return 'Another Role'
