...
def resolve_dmarc_reports(self: DmarcReport, info, **kwargs):
    ...
    if (start_date and end_date) is not None:
        ...
        query = query.filter(Dmarc_Reports.domain_id == domain_id)
        ...
        ).filter(Dmarc_Reports.start_date.between(start_date, end_date))

        if not query.all():
            raise GraphQLError("Error, no reports for that domain in that date range.")
