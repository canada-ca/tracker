...
export const loadWebConnectionsByDomainId =
  ...
  async ({ limit, domainId, startDate, endDate, after, before, offset, orderBy, excludePending = true }) => {
    if (limit === undefined) {
      console.warn(`User: ${userKey} did not set \`limit\` argument for: loadWebConnectionsByDomainId.`)
      throw new Error(i18n._(t`You must provide a \`limit\` value to properly paginate the \`web\` connection.`))
    }

    if (limit <= 0 || limit > 100) {
      console.warn(`User: ${userKey} set \`limit\` argument outside accepted range: loadWebConnectionsByDomainId.`)
      throw new Error(
        i18n._(
    ...
    if (paginationMethodCount > 1) {
      console.warn(`User: ${userKey} set multiple pagination methods for: loadWebConnectionsByDomainId.`)
      throw new Error(
        i18n._(
    ...
    if (excludePending) {
      pendingFilter = aql`
        LET pendingScans = (
          FOR webScan, webScanEdge IN 1 OUTBOUND web webToWebScans
            FILTER webScan.status == "pending"
            RETURN webScan
        )
        FILTER LENGTH(pendingScans) == 0
      `
    }
    ...
    try {
      ...
      console.error(
        `Database error occurred while user: ${userKey} was trying to get cursor for web document with cursor '${
          after || before
