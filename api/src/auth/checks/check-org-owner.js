...
export const checkOrgOwner =
  ...
  async ({ orgId }) => {
    ...
    try {
      ...
      console.error(`Database error when checking to see if user: ${userKey} is the owner of: ${orgId}: ${err}`)
    ...
    try {
      ...
      console.error(`Cursor error when checking to see if user: ${userKey} is the owner of: ${orgId}: ${err}`)
      throw new Error(i18n._(t`Unable to load owner information. Please try again.`))
    }
