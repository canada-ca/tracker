...
export const loadAuditLogs =
  ...
  async ({ after, _before, first, last, orderBy, search }) => {
    ...
    if (typeof first === 'undefined' && typeof last === 'undefined') {
      console.warn(
        `User: ${userKey} did not have either \`first\` or \`last\` arguments set for: loadAuditLogs.`,
      )
      throw new Error(
        i18n._(
      ...
      console.warn(
        `User: ${userKey} attempted to have \`first\` and \`last\` arguments set for: loadAuditLogs.`,
      )
      throw new Error(
        i18n._(
      ...
      if (first < 0 || last < 0) {
        ...
        console.warn(
          `User: ${userKey} attempted to have \`${argSet}\` set below zero for: loadAuditLogs.`,
        )
        throw new Error(
          i18n._(
        ...
        console.warn(
          `User: ${userKey} attempted to have \`${argSet}\` set to ${amount} for: loadAuditLogs.`,
        )
        throw new Error(
          i18n._(
      ...
      console.warn(
        `User: ${userKey} attempted to have \`${argSet}\` set as a ${typeSet} for: loadAuditLogs.`,
      )
      throw new Error(
    ...
    const logKeysQuery = aql`
      WITH auditLogs
        LET logKeys = UNIQUE(FLATTEN(
    ...
    LET hasPreviousPage = (LENGTH(
      ...
      console.error(
        `Database error occurred while user: ${userKey} was trying to query logs in loadAuditLogs, error: ${err}`,
      )
    ...
    try {
      ...
      console.error(
        `Cursor error occurred while user: ${userKey} was trying to gather logs in loadAuditLogs, error: ${err}`,
      )
