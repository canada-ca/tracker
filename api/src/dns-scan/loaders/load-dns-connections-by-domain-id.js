...
export const loadDnsConnectionsByDomainId =
  ...
  async ({ limit, domainId, startDate, endDate, after, before, offset, orderBy }) => {
    if (limit === undefined) {
      console.warn(`User: ${userKey} did not set \`limit\` argument for: loadDnsConnectionsByDomainId.`)
      throw new Error(i18n._(t`You must provide a \`limit\` value to properly paginate the \`DNS\` connection.`))
    }

    if (limit <= 0 || limit > 100) {
      console.warn(`User: ${userKey} set \`limit\` argument outside accepted range: loadDnsConnectionsByDomainId.`)
      throw new Error(
        i18n._(
    ...
    if (paginationMethodCount > 1) {
      console.warn(`User: ${userKey} set multiple pagination methods for: loadDnsConnectionsByDomainId.`)
      throw new Error(
        i18n._(
      ...
      return {
        ...
        pageInfo: {
          hasPreviousPage: !usingRelayExplicitly
            ? false
            : after
            ? dnsScanInfo.hasReversePage
            : dnsScanInfo.hasMoreRelayPage,
