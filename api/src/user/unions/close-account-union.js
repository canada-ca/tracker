...
export const closeAccountUnion = new GraphQLUnionType({
  ...
  description: 'This union is used for the `closeAccount` mutation, to support successful or errors that may occur.',
  types: [closeAccountResult, closeAccountError],
  resolveType({ _type }) {
    if (_type === 'error') {
      return closeAccountError.name
    } else {
      return closeAccountResult.name
    }
  },
})
