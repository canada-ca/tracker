...
export const authenticate = new mutationWithClientMutationId({
  ...
  mutateAndGetPayload: async (
    ...
    if (authenticationCode === user.tfaCode) {
      ...
      // verify user email
      const shouldValidateEmail = sendMethod === 'email' && !user.emailValidated

      // Reset tfa code attempts, set refresh code, and optionally validate email
      await userDataSource.authenticateSetSession({
        userKey: user._key,
        refreshInfo,
        loginDate,
        emailValidated: shouldValidateEmail,
      })
      ...
      // reset tfa code
      await userDataSource.resetTfaCode({ userKey: user._key })
