...
export const closeAccountSelf = new mutationWithClientMutationId({
  ...
  mutateAndGetPayload: async (
    args,
    { i18n, query, collections, transaction, request: { ip }, auth: { userRequired }, dataSources: { user: userDataSource }, validators: { cleanseInput } },
  ) => {
    let submittedUserId
    if (args?.userId) {
    ...
    await userDataSource.removeUserAndAffiliations({ userId })
