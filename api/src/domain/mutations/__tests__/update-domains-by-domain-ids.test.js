...
describe('updateDomainsByDomainIds mutation', () => {
  ...
  describe('user has super admin permission', () => {
    ...
    it('updates domains by domainIds', async () => {
          ...
          mutation {
            updateDomainsByDomainIds(
              input: {
                ...
                tags: ["${tag.tagId}"]
                domainIds: ["${toGlobalId('domains', domain._key)}"]
              }
            ) {
              result {
                ... on DomainBulkResult {
      ...
      expect(response.data.updateDomainsByDomainIds.result.status).toMatch(
  ...
  describe('user does not have permission', () => {
    ...
    it('returns an error message', async () => {
      ...
      expect(response.data.updateDomainsByDomainIds.result.code).toBe(400)
      expect(response.data.updateDomainsByDomainIds.result.description).toMatch(/Permission Denied/)
    })
  })
})
