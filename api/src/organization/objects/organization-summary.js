...
export const organizationSummaryType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    httpsIncludeHidden: {
      ...
      resolve: ({ https, hidden }, _) => {
        const pass = https.pass + hidden.https.pass
        const fail = https.fail + hidden.https.fail
        const total = https.total + hidden.https.total
        ...
        if (total <= 0) {
          ...
          percentPass = Number(((pass / total) * 100).toFixed(1))
          percentageFail = Number(((fail / total) * 100).toFixed(1))
        }
