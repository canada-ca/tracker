  ...
  async ({
    ...
    if (typeof first === 'undefined' && typeof last === 'undefined') {
      ...
      if (first < 0 || last < 0) {
        ...
        console.warn(`User: ${userKey} attempted to have \`${argSet}\` set below zero for: loadOrgConnectionsByUserId.`)
        throw new Error(i18n._(t`\`${argSet}\` on the \`Organization\` connection cannot be less than zero.`))
      } else if (first > 100 || last > 100) {
        const argSet = typeof first !== 'undefined' ? 'first' : 'last'
        const amount = typeof first !== 'undefined' ? first : last
        console.warn(`User: ${userKey} attempted to have \`${argSet}\` to ${amount} for: loadOrgConnectionsByUserId.`)
        throw new Error(
          i18n._(
