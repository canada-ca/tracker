...
def upgrade():
    ...
    op.create_foreign_key(
        "dkim_scans_id_fkey", "dkim_scans", "mail_scans", ["id"], ["id"]
    )
    op.drop_constraint("dmarc_scans_id_fkey", "dmarc_scans", type_="foreignkey")
    op.create_foreign_key(
        "dmarc_scans_id_fkey", "dmarc_scans", "mail_scans", ["id"], ["id"]
    )
    op.drop_column("dmarc_scans", "dmarc_phase")
    op.add_column(
        "domains", sa.Column("selectors", postgresql.ARRAY(sa.String()), nullable=True)
    )
    op.drop_constraint("https_scans_id_fkey", "https_scans", type_="foreignkey")
    op.create_foreign_key(
        "https_scans_id_fkey", "https_scans", "web_scans", ["id"], ["id"]
    )
    ...
    op.create_foreign_key(
        "spf_scans_id_fkey", "spf_scans", "mail_scans", ["id"], ["id"]
    )
...
def downgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["domain_id"], ["domains.id"], name="scans_domain_id_fkey"
        ),
        sa.ForeignKeyConstraint(
            ["initiated_by"], ["users.id"], name="fk_users_id_scans"
        ),
    ...
    op.add_column(
        "dmarc_scans",
        sa.Column("dmarc_phase", sa.INTEGER(), autoincrement=False, nullable=True),
    )
    op.drop_constraint("dmarc_scans_id_fkey", "dmarc_scans", type_="foreignkey")
