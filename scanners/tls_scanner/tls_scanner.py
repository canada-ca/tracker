...
from sslyze import Scanner, ServerScanRequest, ServerNetworkLocation, \
    ServerHostnameCouldNotBeResolved, TlsVersionEnum
...
def run_scan(domain, ip_address=None):
    try:
        # Use all possible scans except http_headers. We will implement this ourselves to better fit our use-case.
        desired_scans = {ScanCommand.CERTIFICATE_INFO, ScanCommand.SESSION_RESUMPTION,
                         ...
                         ScanCommand.TLS_COMPRESSION, ScanCommand.TLS_1_3_EARLY_DATA, ScanCommand.OPENSSL_CCS_INJECTION,
                         ScanCommand.TLS_FALLBACK_SCSV, ScanCommand.HEARTBLEED, ScanCommand.ROBOT,
                         ScanCommand.SESSION_RENEGOTIATION, ScanCommand.ELLIPTIC_CURVES}
        scan_requests = [ServerScanRequest(server_location=ServerNetworkLocation(hostname=domain, ip_address=ip_address),
                                           scan_commands=desired_scans)]
    ...
    for result in scanner.get_results():
        print(json.dumps(obj_to_dict(result), indent=4, sort_keys=True))
