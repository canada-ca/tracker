...
def test_get_all_summaries(mocker, all_summaries_input, all_summaries_output):
    mocker.patch("tracker_client.client.execute_query", auto_spec=True, return_value=all_summaries_input)
...
def test_get_summary_by_name(mocker, name_summary_input, org_summary_output):
    mocker.patch("tracker_client.client.execute_query", auto_spec=True, return_value=name_summary_input)
    expected_params = {"orgSlug": "foo-bar"}

    client = Client()
    result = tc.get_summary_by_name(client, "foo bar")
    tc.execute_query.assert_called_once_with(client, queries.SUMMARY_BY_SLUG, expected_params)
    assert result == json.dumps(org_summary_output, indent=4)
...
def test_get_domain_results(mocker, scan_results_input, scan_results_output):
    mocker.patch("tracker_client.client.execute_query", auto_spec=True, return_value=scan_results_input)
    expected_params = {"domain": "foo.bar"}

    client = Client()
    result = tc.get_domain_results(client, "foo.bar")
    tc.execute_query.assert_called_once_with(client, queries.DOMAIN_RESULTS, expected_params)
...
def test_get_domain_status(mocker, domain_status_input, domain_status_output):
    mocker.patch("tracker_client.client.execute_query", auto_spec=True, return_value=domain_status_input)
    expected_params = {"domain": "foo.bar"}

    client = Client()
    result = tc.get_domain_status(client, "foo.bar")
    tc.execute_query.assert_called_once_with(client, queries.DOMAIN_STATUS, expected_params)
