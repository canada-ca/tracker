  ...
  try {
    httpsGuidanceTagInfoCursor = await query`
      ...
      LET hasNextPage = (LENGTH(
        FOR tag IN httpsGuidanceTags
          FILTER tag._key IN ${httpsGuidanceTags}
          FILTER TO_NUMBER(REGEX_SPLIT(tag._key, "[A-Za-z]+")[1]) > TO_NUMBER(REGEX_SPLIT(LAST(retrievedHttpsGuidanceTags)._key, "[A-Za-z]+")[1])
      ...
      LET hasPreviousPage = (LENGTH(
        FOR tag IN httpsGuidanceTags
          FILTER tag._key IN ${httpsGuidanceTags}
          FILTER TO_NUMBER(REGEX_SPLIT(tag._key, "[A-Za-z]+")[1]) < TO_NUMBER(REGEX_SPLIT(FIRST(retrievedHttpsGuidanceTags)._key, "[A-Za-z]+")[1])
          SORT TO_NUMBER(REGEX_SPLIT(tag._key, "[A-Za-z]+")[1]) ${sortString} LIMIT 1
          RETURN tag
      ) > 0 ? true : false)
