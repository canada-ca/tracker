  ...
  try {
    spfGuidanceTagInfoCursor = await query`
      ...
      LET hasNextPage = (LENGTH(
        FOR tag IN spfGuidanceTags
          FILTER tag._key IN ${spfGuidanceTags}
          FILTER TO_NUMBER(REGEX_SPLIT(tag._key, "[A-Za-z]+")[1]) > TO_NUMBER(REGEX_SPLIT(LAST(retrievedSpfGuidanceTags)._key, "[A-Za-z]+")[1])
      ...
      LET hasPreviousPage = (LENGTH(
        FOR tag IN spfGuidanceTags
          FILTER tag._key IN ${spfGuidanceTags}
          FILTER TO_NUMBER(REGEX_SPLIT(tag._key, "[A-Za-z]+")[1]) < TO_NUMBER(REGEX_SPLIT(FIRST(retrievedSpfGuidanceTags)._key, "[A-Za-z]+")[1])
          SORT TO_NUMBER(REGEX_SPLIT(tag._key, "[A-Za-z]+")[1]) ${sortString} LIMIT 1
          RETURN tag
      ) > 0 ? true : false)
