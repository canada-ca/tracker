...
const removeDomain = new mutationWithClientMutationId({
  ...
  mutateAndGetPayload: async (
    ...
    if (typeof domain === 'undefined') {
      console.warn(
        `User: ${userKey} attempted to remove domain: ${domainId} however no domain is associated with that id.`,
      )
    ...
    if (typeof org === 'undefined') {
      console.warn(
        `User: ${userKey} attempted to remove domain: ${domain._key} in org: ${orgId} however there is no organization associated with that id.`,
      )
      throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
    }
    ...
    if (org.verified) {
      console.warn(
        `User: ${userKey} attempted to remove a domain from an organization: ${org._key} that is verified.`,
      )
      throw new Error(
        i18n._(
          t`Unable to remove domains belonging to verified organizations.`,
        ),
      )
    }
    ...
    if (permission !== 'super_admin' && permission !== 'admin') {
      console.warn(
        `User: ${userKey} attempted to remove domain: ${domain._key} in org: ${org._key} however they do not have permission in that org.`,
      )
      throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
    }
    ...
    try {
      ...
      console.error(
        `Database error occurred for user: ${userKey}, when counting domain claims for domain: ${domain._key}, error: ${err}`,
      )
      throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
    }
    ...
    if (countCursor.count <= 1) {
      ...
      try {
        ...
        console.error(
          `Transaction run error occurred for user: ${userKey} when removing scan data for domain: ${domain._key}, error: ${err}`,
        )
        throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
      }
      ...
      try {
        ...
        console.error(
          `Transaction run error occurred for user: ${userKey} removing domain: ${domain._key} from org: ${org._key}, error: ${err}`,
        )
        throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
      }
    } else {
      try {
        ...
        console.error(
          `Transaction run error occurred for user: ${userKey} removing claim for domain: ${domain._key} in org: ${org._key}, error: ${err}`,
        )
        throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
      }
    }
    ...
    try {
      ...
      console.error(
        `Transaction commit error occurred for user: ${userKey} removing domain: ${domain._key} from org: ${org._key}, error: ${err}`,
      )
      throw new Error(i18n._(t`Unable to remove domain. Please try again.`))
    }

    console.info(
      `User: ${userKey} successfully removed domain: ${domain._key} from org: ${org._key}.`,
    )
    return {
      status: i18n._(
