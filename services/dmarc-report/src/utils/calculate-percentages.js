const calculatePercentages = ({ fail, pass, passDkimOnly, passSpfOnly }) => {
  const total = [fail, pass, passDkimOnly, passSpfOnly].reduce((a, b) => a + b, 0)
  ...
  const passDkimOnlyPercentage = Number(passDkimOnly <= 0 ? 0 : Number((passDkimOnly / total) * 100).toFixed(1))

  const passSpfOnlyPercentage = Number(passSpfOnly <= 0 ? 0 : Number((passSpfOnly / total) * 100).toFixed(1))
